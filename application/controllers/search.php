	...
	public function index($page = 1) 
	{
		...
		if (is_array($keywords) AND !empty($keywords)) 
		{
			...
			foreach($keywords as $value)
			{
				if ( ! in_array($value,$stop_words) AND !empty($value))
				{
					...
					$keyword_string = $keyword_string.$plus."(CASE WHEN incident_NoOfTrees LIKE '%$chunk%' THEN 2 ELSE 0 END) + "
										. "(CASE WHEN incident_remarks LIKE '%$chunk%' THEN 1 ELSE 0 END) ";
										
					$where_string = $where_string.$or."(incident_NoOfTrees LIKE '%$chunk%' OR incident_remarks LIKE '%$chunk%')";
					$i++;
				}
			}
		...
		if ( ! empty($search_query))
		{
			...
			foreach ($query as $search)
			{
				...
				$incident_NoOfTrees = $search->incident_NoOfTrees;
				$highlight_title = "";
				$incident_title_arr = explode(' ', $incident_NoOfTrees); 
				...
				$incident_remarks = $search->incident_remarks;

				// Remove any markup, otherwise trimming below will mess things up
				$incident_remarks = strip_tags($incident_remarks);

				// Trim to 180 characters without cutting words
				if ((strlen($incident_remarks) > 180) AND (strlen($incident_remarks) > 1))
				{
					$whitespaceposition = strpos($incident_remarks," ",175)-1;
					$incident_remarks = substr($incident_remarks, 0, $whitespaceposition);
				}
                
				$highlight_description = "";
				$incident_description_arr = explode(' ', $incident_remarks);
